 ...
 *
 * TRANSACTION FLOW: All write operations (transfers, approvals, contract writes) return
 * unsigned transaction data instead of executing transactions. Clients must:
 * 1. Receive the transaction request object (to, from, data, value, chainId)
 * 2. Sign the transaction with their wallet
 * 3. Submit the signed transaction to the network
 *
 * All transaction builder tools require a 'from' address parameter to specify the sender.
...
export function registerEVMTools(server: McpServer) {
    ...
    "get_chain_info",
    {
      ...
      inputSchema: {
        network: z
          ...
          .optional()
          .describe(
            "Network name (e.g., 'ethereum', 'optimism', 'arbitrum', 'base') or chain ID. Defaults to Ethereum mainnet."
          ),
      },
      annotations: {
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error fetching gas prices: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
        ...
        if (!ensName.includes(".")) {
          return {
            content: [
              {
                type: "text",
                text: `Error: "${ensName}" is not a valid ENS name. ENS names must contain a dot (e.g., 'name.eth').`,
              },
            ],
            isError: true,
          };
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error looking up ENS name: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
  ...
  server.registerTool(
    "get_block",
    {
      ...
      inputSchema: {
        blockIdentifier: z
          .string()
          .describe("Block number (as string) or block hash"),
        network: z
          .string()
          .optional()
          .describe("Network name or chain ID. Defaults to Ethereum mainnet."),
    ...
    async ({ blockIdentifier, network = "ethereum" }) => {
      try {
        ...
        if (blockIdentifier.startsWith("0x") && blockIdentifier.length === 66) {
          // It's a hash
          block = await services.getBlockByHash(
            blockIdentifier as Hash,
            network
          );
        } else {
          // It's a number
          block = await services.getBlockByNumber(
    ...
    "get_allowance",
    {
      ...
      inputSchema: {
        ...
        spenderAddress: z
          .string()
          .describe(
            "The address allowed to spend the token (usually a contract address)"
          ),
        ownerAddress: z.string().describe("The owner address"),
        network: z
          .string()
          .optional()
          .describe("Network name or chain ID. Defaults to Ethereum mainnet."),
      ...
      try {
              ...
              text: JSON.stringify(
                {
                  ...
                  message:
                    allowance === 0n ? "No allowance set" : "Allowance is set",
                },
                null,
                2
              ),
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error fetching allowance: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
  ...
  server.registerTool(
    ...
    async ({ txHash, network = "ethereum" }) => {
      try {
        ...
        return {
          content: [
            { type: "text", text: services.helpers.formatJson(receipt) },
          ],
        };
      } catch (error) {
        return {
    ...
    "wait_for_transaction",
    {
      ...
      inputSchema: {
        ...
        confirmations: z
          .number()
          .optional()
          .describe("Number of block confirmations required. Defaults to 1."),
        network: z
          .string()
          .optional()
          .describe("Network name or chain ID. Defaults to Ethereum mainnet."),
  ...
  server.registerTool(
    ...
    async ({ contractAddress, network = "ethereum" }) => {
      try {
        const abi = await services.fetchContractABI(
              ...
              text: JSON.stringify(
                {
                  ...
                  abiFormat: "json",
                  readableFunctions,
                  totalFunctions: parsed.filter((i) => i.type === "function")
                    .length,
                  abi: parsed,
                },
                null,
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error fetching ABI: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
    ...
    "read_contract",
    {
      ...
      inputSchema: {
        ...
        functionName: z
          .string()
          .describe(
            "Function name (e.g., 'name', 'symbol', 'balanceOf', 'totalSupply', 'owner')"
          ),
        args: z
          .array(z.string())
          .optional()
          .describe(
            "Function arguments as strings (e.g., ['0xAddress'] for balanceOf)"
          ),
        abiJson: z
          .string()
          .optional()
          .describe(
            "Full contract ABI as JSON string (optional - will auto-fetch verified contract ABI if not provided)"
          ),
        network: z
          .string()
          .optional()
    ...
    async ({
      ...
      try {
        ...
        if (abiJson) {
          ...
          try {
            const fetchedAbi = await services.fetchContractABI(
              contractAddress as Address,
              network
            );
  ...
  server.registerTool(
    "build_write_contract",
    {
      description:
        "Build a contract write transaction. Returns unsigned transaction data to be signed and submitted by the client. Automatically fetches ABI from block explorer if not provided (requires ETHERSCAN_API_KEY). Use this to prepare any write function call on verified contracts.",
      inputSchema: {
        contractAddress: z.string().describe("The contract address"),
        from: z.string().describe("Sender address or ENS name"),
        functionName: z
          .string()
          .describe(
            "Function name to call (e.g., 'mint', 'swap', 'stake', 'approve')"
          ),
        args: z
          .array(z.string())
          .optional()
          .describe(
            "Function arguments as strings (e.g., ['0xAddress', '1000000'])"
          ),
        value: z
          .string()
          .optional()
          .describe(
            "ETH value to send with transaction in ether (e.g., '0.1' for payable functions)"
          ),
        abiJson: z
          .string()
          .optional()
    ...
    async ({
      ...
      try {
        const fromAddress = await services.resolveAddress(from, network);
        const client = await services.getPublicClient(network);
        ...
        const txRequest = await services.writeContract(
          fromAddress,
          writeParams,
          network
        );
              ...
              text: JSON.stringify(
                {
                  ...
                  serializedTransaction: txRequest.serialized,
                  abiSource: abiJson ? "provided" : "auto-fetched",
                  message:
                    "Transaction prepared. Sign and submit this transaction with your wallet.",
                },
                null,
                2
              ),
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error building contract write transaction: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
    ...
    "multicall",
    {
      ...
      inputSchema: {
          ...
          .array(
            z.object({
              ...
              abiJson: z
                ...
                .optional()
                .describe(
                  "Contract ABI as JSON string (optional - will auto-fetch if not provided)"
                ),
            })
          )
        ...
        allowFailure: z
          ...
          .optional()
          .describe(
            "If true, returns partial results even if some calls fail. Defaults to true."
          ),
        network: z
          .string()
          .optional()
          ...
          calls.map(async (call) => {
            ...
            if (call.abiJson) {
              try {
                ...
                functionAbi = services.getFunctionFromABI(
                  abi,
                  call.functionName
                );
              } catch (error) {
                throw new Error(
                  `Error parsing ABI for ${call.contractAddress}: ${error instanceof Error ? error.message : String(error)}`
                );
              }
            } else {
              ...
              try {
                const fetchedAbi = await services.fetchContractABI(
                  call.contractAddress as Address,
                  network
                );
                abi = services.parseABI(fetchedAbi);
                functionAbi = services.getFunctionFromABI(
                ...
                if (!commonFunctions[call.functionName]) {
                  throw new Error(
                    `Could not auto-fetch ABI for ${call.contractAddress}. Function '${call.functionName}' not in common signatures. Please provide abiJson parameter.`
                  );
                }
              ...
              text: JSON.stringify(
                {
                  ...
                  successfulCalls: formattedResults.filter(
                    (r: any) => r.status === "success"
                  ).length,
                  failedCalls: formattedResults.filter(
                    (r: any) => r.status === "failure"
                  ).length,
  ...
  // ============================================================================
  // TRANSACTION BUILDER TOOLS (Prepare unsigned transactions)
  // ============================================================================
  ...
  server.registerTool(
    ...
    {
      description:
        "Build a native token (ETH, MATIC, etc.) transfer transaction. Returns unsigned transaction data to be signed and submitted by the client.",
      inputSchema: {
        from: z.string().describe("Sender address or ENS name"),
        to: z.string().describe("Recipient address or ENS name"),
        amount: z
          .string()
          .describe("Amount to send in ether (e.g., '0.5' for 0.5 ETH)"),
        network: z
          .string()
          .optional()
          .describe("Network name or chain ID. Defaults to Ethereum mainnet."),
    ...
    async ({ from, to, amount, network = "ethereum" }) => {
      try {
        const fromAddress = await services.resolveAddress(from, network);
        const txRequest = await services.transferETH(
          fromAddress,
          to as Address,
          amount,
          network
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error building native token transfer: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
  ...
  server.registerTool(
    ...
    {
      description:
        "Build an ERC20 token transfer transaction. Returns unsigned transaction data to be signed and submitted by the client.",
      inputSchema: {
        tokenAddress: z.string().describe("The ERC20 token contract address"),
        from: z.string().describe("Sender address or ENS name"),
        to: z.string().describe("Recipient address or ENS name"),
        amount: z
          .string()
          .describe("Amount to send (in token units, accounting for decimals)"),
        network: z
          .string()
          .optional()
          .describe("Network name or chain ID. Defaults to Ethereum mainnet."),
    ...
    async ({ tokenAddress, from, to, amount, network = "ethereum" }) => {
      try {
        const fromAddress = await services.resolveAddress(from, network);
              ...
              text: JSON.stringify(
                {
                  ...
                  serializedTransaction: result.serialized,
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error building ERC20 transfer: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
  ...
  server.registerTool(
    ...
    {
      description:
        "Build an ERC20 token approval transaction. Returns unsigned transaction data to be signed and submitted by the client. Required before interacting with DEXes, lending protocols, etc.",
      inputSchema: {
        tokenAddress: z.string().describe("The ERC20 token contract address"),
        from: z.string().describe("Owner address or ENS name"),
        spenderAddress: z
          .string()
          .describe(
            "The address that will be allowed to spend tokens (usually a contract)"
          ),
        amount: z
          .string()
          .describe(
            "Amount to approve (in token units). Use '0' to revoke approval."
          ),
        network: z
          .string()
          .optional()
      ...
      try {
              ...
              text: JSON.stringify(
                {
                  ...
                  approvalAmount: result.amount.formatted,
                  symbol: result.token.symbol,
                  decimals: result.token.decimals,
                  message:
                    "Approval transaction prepared. Sign and submit this transaction with your wallet.",
                },
                null,
                2
              ),
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error building token approval: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Error fetching ERC1155 balance: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
